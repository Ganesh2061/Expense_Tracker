...
export function LoginSignupModal({
  ...
  return <div className="fixed inset-0 bg-gray-50 flex items-center justify-center z-50">
      <div className="bg-white rounded-lg shadow-xl p-8 w-full max-w-md">
        ...
        <div className="flex border-b border-gray-200 mb-6">
          <button className={`pb-2 px-4 ${isLogin ? 'border-b-2 border-teal-500 text-teal-600' : 'text-gray-500'}`} onClick={() => setIsLogin(true)}>
            Login
          </button>
          <button className={`pb-2 px-4 ${!isLogin ? 'border-b-2 border-teal-500 text-teal-600' : 'text-gray-500'}`} onClick={() => setIsLogin(false)}>
            Sign Up
          </button>
        </div>
        <form onSubmit={handleSubmit}>
          {!isLogin && <div className="mb-4">
              ...
              <input type="text" name="name" value={formData.name} onChange={handleChange} className="w-full p-2 border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-teal-500" required={!isLogin} />
            </div>}
          <div className="mb-4">
            <label className="block text-gray-700 text-sm font-medium mb-1">
              Email
          ...
          <button type="submit" className="w-full py-2 bg-teal-500 text-white rounded hover:bg-teal-600">
            {isLogin ? 'Login' : 'Sign Up'}
          </button>
        </form>
        {isLogin && <p className="mt-4 text-center text-sm text-gray-600">
